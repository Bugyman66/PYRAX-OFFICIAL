...
import { useNodeStore, BootnodeInfo } from '../stores/nodeStore';
...
export default function NetworkNodes() {
  const { bootnodes, fetchBootnodes, status } = useNodeStore();
  ...
  useEffect(() => {
    if (status?.connected) {
      fetchBootnodes();
      // Refresh bootnode info every 30 seconds
      const interval = setInterval(fetchBootnodes, 30000);
      return () => clearInterval(interval);
    }
  }, [status?.connected, fetchBootnodes]);
