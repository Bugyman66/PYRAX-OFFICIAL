...
export default function EVMContractsPage() {
  ...
  const fetchContracts = useCallback(async (page: number, pageSize: number) => {
    ...
    try {
      const response = await fetch(`/api/contracts?page=${page}&pageSize=${pageSize}&type=evm`)
      ...
      <div className="sm:flex sm:items-center sm:justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg bg-pyrax-500/10">
            <CodeBracketIcon className="h-6 w-6 text-pyrax-500" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-white">EVM Contracts</h1>
            <p className="text-sm text-stone-400">
              {pagination.totalContracts > 0 
                ? `${formatNumber(pagination.totalContracts)} contracts deployed`
                : 'Browse all EVM smart contracts on the chain'}
            </p>
          </div>
        </div>
                  ...
                  >
                    <td className="px-4 py-4 whitespace-nowrap">
                      <Link 
                        ...
                        className="flex items-center gap-2 text-pyrax-500 hover:text-pyrax-400 font-mono text-sm"
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        ...
                        <Link 
                          ...
                          className="text-xs text-pyrax-500 hover:text-pyrax-400"
                        >
