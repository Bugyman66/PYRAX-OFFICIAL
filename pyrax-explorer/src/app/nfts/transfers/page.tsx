...
export default function NFTTransfersPage() {
  ...
  const fetchTransfers = useCallback(async (page: number, pageSize: number, currentFilters: Filters) => {
    ...
    try {
      ...
      const response = await fetch(`/api/nfts/transfers?${params}`)
      ...
      setError('Failed to fetch NFT transfers')
      console.error(err)
    } finally {
      setLoading(false)
    }
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="sm:flex sm:items-center sm:justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">NFT Transfers</h1>
            <p className="text-sm text-stone-400">
              {pagination.totalTransfers > 0 
                ? `${formatNumber(pagination.totalTransfers)} NFT transfers`
                : 'Browse all NFT transfers on the chain'}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 sm:mt-0 flex items-center gap-4">
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
              showFilters ? 'bg-purple-500 text-white' : 'bg-stone-800 text-white hover:bg-stone-700'
            }`}
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="0x..."
                  className="w-full rounded-lg bg-stone-800 border border-stone-700 pl-3 pr-9 py-2 text-sm text-white font-mono focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
                {addressInput && (
                  <button
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-lg bg-stone-800 border border-stone-700 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
              >
                {NFT_TYPES.map((type) => (
              ...
              {loading ? (
                ...
                Array.from({ length: pagination.pageSize || 10 }).map((_, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 bg-stone-800 rounded-lg animate-pulse" />
                        <div className="space-y-1">
                          <div className="h-4 w-20 bg-stone-800 rounded animate-pulse" />
                          <div className="h-3 w-12 bg-stone-800 rounded animate-pulse" />
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                ...
                transfers.map((transfer, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <Link 
                        href={`/nft/${transfer.collectionAddress}/${transfer.tokenId}`}
                        className="flex items-center gap-3 group"
                      >
                        <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-purple-400 to-pink-500 flex items-center justify-center overflow-hidden">
                          {transfer.imageUrl ? (
                            <img 
                              src={transfer.imageUrl} 
                              alt={`${transfer.collectionName} #${transfer.tokenId}`}
                              className="h-full w-full object-cover"
                            />
                          ) : (
                            <PhotoIcon className="h-5 w-5 text-white/70" />
                          )}
                        </div>
                        <div>
                          <div className="text-sm font-medium text-white group-hover:text-purple-400 transition-colors">
                            {transfer.collectionName || 'Unknown Collection'}
                          </div>
                          <div className="text-xs text-stone-500">
                            {formatTokenId(transfer.tokenId)}
                          </div>
                        </div>
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <ArrowLongRightIcon className="h-4 w-4 text-purple-500" />
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      <Link 
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                        transfer.tokenType === 'ERC721' ? 'bg-purple-500/10 text-purple-400' :
                        transfer.tokenType === 'ERC1155' ? 'bg-green-500/10 text-green-400' :
                        transfer.tokenType === 'WASM721' ? 'bg-pink-500/10 text-pink-400' :
